...
def product_create():
    ...
    if image_file:
        ...
        # ✅ sanitize and ensure folder exists before saving
        file_name = secure_filename(image_file.filename)
        upload_folder = os.path.join(current_app.root_path, "static", "image", "product")
...
def watermark_image(image_path, watermark_text="© Socheat mart Products"):
...
def product_delete():
    ...
    try:
        if product.image:
            image_path = os.path.join(current_app.root_path, "static", "image", "product", product.image)
