...
# better: import db from extension to avoid circular imports with app.py
...
def user_create():
    ...
    if profile_file:
        ...
        # build upload folder inside project
        upload_folder = os.path.join(current_app.root_path, "static", "image", "user")
        os.makedirs(upload_folder, exist_ok=True)  # <-- THIS fixes FileNotFoundError
    ...
    user = User(
        ...
        password=hashed_pw,
        email=email,
        profile=file_name,           # we store only the filename
...
def watermark_image(image_path, watermark_text="© socheatmart"):
